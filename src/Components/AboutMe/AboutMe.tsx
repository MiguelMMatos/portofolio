import "./aboutme.css";

function AboutMe() {
  return (
    <div id="About" className="content">
      <div className="about_container">
        <div className="content_tab">/ about me</div>
        <div className="inner_content">
          I am currently a Software Development Engineer at Amazon, working in
          the AWS sector under team Route 53. At the same time, I am undertaking
          a part-time Master's of Science in Software Engineering at University
          of Oxford. Here are some technologies I have been working with:
          Typescript Python React.js Java Javascript ES6+ C# Outside of work,
          I'm interested in following the developments of science. I also play a
          lot of video games. And make TikToks.
        </div>
      </div>
    </div>
  );
}

export default AboutMe;
